...
def resize_lora_layer_svd(
    ...
    **kwargs
    ...
    # 6. Update the layer structure
    # FIX: Remove 'init_lora_weights' from kwargs to avoid multiple values error
    # We enforce True here to ensure tensors are created, which we then overwrite.
    ...
    # 7. Overwrite with SVD-preserved weights
    with torch.no_grad():
        if k < new_rank:
             layer.lora_A[adapter_name].weight.data.zero_()
             layer.lora_B[adapter_name].weight.data.zero_()
